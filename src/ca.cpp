...
{
  double somatic_rate=0;
  double min_diversity=1000;
  double min_shape=1000;
  ...
  //Going to try adding in number of new cells
  double n_new_cells;
  if (n_new_cells > 64)
  ...
  for (unsigned int j=0; j < shape_fitness_list.size(); ++j) 
    if (shape_fitness_list.at(j) < min_shape)
      min_shape = shape_fitness_list.at(j);



  if (par.print_fitness)
    cout << "Type diversity: " << min_diversity << "  Shape uniqueness: "<< min_shape << endl; //  "Final fitness: " << min_diversity + somatic_rate << endl;

  // final fitness value to return, have turned off germ-soma thing or now. 
  // return min_diversity + somatic_rate + n_new_cells; 

  
  return min_shape + min_diversity / 2;
...
double CellularPotts::CalculateShape()
{
  ...
  double shape = (dev * 3.) + sqrt(whitespacex + whitespacey);
...
// Function to ensure all cells are connected indirectly to all cells on lattice  
