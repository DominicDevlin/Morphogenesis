...
double angle2_calculator(pair<int,int> point1, pair<int,int> point2, pair<int,int> center)
{
  ...
  // Adjusting angle to the range [0, 2pi] for clockwise direction
  if (angle < 0) {
      angle += 2 * M_PI;
  }
...
void CellularPotts::WetRandomCells()
{
  vector<Cell>::iterator i;
  for ( (i=cell->begin(),i++); i!=cell->end(); i++)
  {
...
{
  int curr_length = WettingLength();
  if (curr_length < init_wet_length * 0.9)
  {
    return true;
  }
  else
